...
const data = await mkdtemp(resolve(tmpdir(), 'modelpath-service-smoke-'));
...
async function start() {
  child = spawn(process.execPath, ['server/content-host.mjs', '--host', '127.0.0.1', '--port', String(port), '--public-host', '100.100.40.76'], { cwd: root, env: { ...process.env, MODELPATH_DATA_DIR: data, MODELPATH_AI_DISABLED: '1' }, stdio: 'ignore' });
...
function call(path, body, token = tokenA, origin = `http://100.100.40.76:${port}`) {
  return new Promise((resolvePromise, reject) => {
    const req = httpRequest({ hostname: '127.0.0.1', port, path, method: body === undefined ? 'GET' : 'POST', headers: { Host: `100.100.40.76:${port}`, Origin: origin, 'Content-Type': 'application/json', 'X-ModelPath-Identity': token } }, res => {
...
try {
  ...
  const status = (await call('/api/content/status')).value; assert.equal(status.version, manifest.version); assert.equal(status.publicUrl, `http://100.100.40.76:${port}/`);
  ...
  assert.equal((await call('/service-data/modelpath.sqlite')).status, 404);
