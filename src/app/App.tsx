...
function AppRoutes() {
  ...
  if (!session && !error) return <div className="session-loading" role="status" aria-live="polite"><div className="session-loading-orbit" aria-hidden="true"><i/><i/><i/></div><div className="session-loading-content"><BrandIcon/><strong>CeptLens</strong><small>{uiText(locale,"正在准备你的学习空间…","Preparing your learning space…")}</small><span className="session-loading-progress" aria-hidden="true"/></div></div>;
  if (!session?.authenticated && sessionStorage.getItem("ceptlens.visitor-entry.v1") !== "true") return <Navigate to={`/sign-in?next=${encodeURIComponent(location.pathname+location.search)}`} replace/>;
