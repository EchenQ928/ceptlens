...
export function LearningCompanion() {
  ...
  return <>
    ...
    {panel === "assistant" && !disabled && <SidePanel key="assistant" title={t("学习助手", "Learning assistant")} subtitle={t("解释概念 · 拆解问题 · 继续追问", "Explain concepts · break down problems · ask follow-ups")} close={() => setPanel(null)}><AssistantPanel reference={reference} clearReference={() => setReference(null)} /></SidePanel>}
    {panel === "profile" && <SidePanel key="profile" title={t("我的讨论身份", "My discussion identity")} close={() => setPanel(null)}><form className="drawer-body identity-form" onSubmit={async e => { e.preventDefault(); setProfileBusy(true); try { await rename(name); setPanel(null); setError(""); } catch (e) { setError(e instanceof Error ? e.message : t("保存失败", "Save failed")); } finally { setProfileBusy(false); } }}><UserRound size={32} /><label>{t("显示名", "Display name")}<input autoFocus value={name} maxLength={40} placeholder={t("让同事知道你是谁", "Help colleagues know who you are")} onChange={e => setName(e.target.value)} /></label><p>{t("登录后可跨设备同步学习进度、答卷和助手记录。", "Sign in to sync progress, papers, and assistant history across devices.")}</p>{(error || sessionError) && <p className="service-error" role="alert">{error || sessionError}</p>}<button className="primary-button" disabled={profileBusy || !name.trim()}>{t("保存显示名", "Save display name")}</button></form></SidePanel>}
    {panel === "auth" && <SidePanel key="auth" title={t("登录 CeptLens", "Sign in to CeptLens")} close={() => setPanel(null)}><AuthPanel close={() => { setPanel(null); void refreshSession(); }} /></SidePanel>}
    </AnimatePresence>
  </>;
}
