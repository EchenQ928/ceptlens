...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`app-frame ${location.pathname === "/" ? "aura-landing" : "aura-workspace"}`}>
    ...
    <header className="topbar">
      ...
      <LiquidSelection navigation className="top-navigation" value={location.pathname+locale} label={uiText(locale, "主导航", "Main navigation")}>
        {navItems.filter(item => item.to !== "/developer" || session?.user.role === "developer").map(({ to, zh, en, kind }) => <NavLink key={to} to={to}>
          <ProductIcon kind={kind}/><span>{uiText(locale, zh, en)}</span>
        </NavLink>)}
      </LiquidSelection>
