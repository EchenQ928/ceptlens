...
export function ExamIntroduction({ catalog, name, setName, busy, connected, error, start, open }: { catalog: ExamCatalog | null; name: string; setName: (s: string) => void; busy: boolean; connected: boolean; error: string; start: () => void; open: (id: string) => void }) {
  ...
  return <div className="page assessment-page">
    ...
    <div className="assessment-start-grid"><section className="assessment-card"><h2>{t("本次考核", "Your assessment")}</h2>
      ...
      <details className="assessment-details"><summary>{t("评分与考核规则", "Scoring & assessment rules")}</summary><ul className="assessment-rules"><li><CheckCircle2 size={18}/>{t("客观题每题 5 分，多选需全部正确。", "5 points per objective question; multiple choice requires all correct options.")}</li><li><LockKeyhole size={18}/>{t("考核期间停用助手与共享批注。", "The assistant and shared annotations are disabled during assessment.")}</li></ul><p>{t("主观题每题 25 分。题型未齐或评分未完成，不发布完整总成绩。", "25 points per short answer. Full scores are withheld until all question types and grading are complete.")}</p></details>
      <label className="candidate-name">{t("提交人", "Candidate name")}<input value={name} onChange={e => setName(e.target.value)} maxLength={40} placeholder={t("填写你的显示名", "Enter your display name")} /></label>
    ...
    </section><aside className="assessment-card exam-history"><h2>{t("历史记录", "History")}</h2>
      {!catalog?.exams.length ? <div className="companion-empty"><ClipboardCheck size={28} /><p>{t("还没有考核记录", "No attempts yet")}</p></div> : catalog.exams.map(e => <button key={e.id} onClick={() => open(e.id)}><span>{date(e.startedAt, locale)}</span><b>{e.status === "active" ? t("进行中 · 继续作答", "In progress · Continue") : e.total === null ? t(`已交卷 · 客观题 ${e.objectiveScore ?? 0} 分`, `Submitted · Objective score ${e.objectiveScore ?? 0}`) : `${e.total} / 100 ${t("分", "points")}`}</b><small>{e.status === "submitted" && e.total === null ? t("完整总成绩暂不发布", "Full score not yet published") : ""}</small></button>)}
    </aside></div>
  </div>;
}
...
export function ExamPaper({ attempt, answers, index, setIndex, remaining, busy, error, saveStatus, answer, save, submit, reload }: {
  ...
  return <div className="page assessment-page">
    ...
    <div className="active-paper-grid"><motion.article key={q.id} className="assessment-card exam-question" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .3, ease: [.22, 1, .36, 1] }}><div className="exam-question-meta">{examTypeName(q.type, locale)} · {q.type === "subjective" ? 25 : 5} {t("分", "points")}{q.type === "multiple_choice" ? t(" · 全部正确才得分", " · Every option must be correct") : ""}</div><h2>{plain(q.stem)}</h2>
    ...
    </motion.article><aside className="assessment-card answer-sheet"><h2>{t("答题卡", "Answer sheet")}</h2><p>{t(`已作答 ${answeredCount(answers)} / ${attempt.questions.length}`, `Answered ${answeredCount(answers)} / ${attempt.questions.length}`)}</p>
