import { CeptCheckMark } from "./CeptCheckMark";
  ...
  return (
    <article className="question-panel">
      <div className="question-toolbar" data-annotation-ignore>
        <div className="question-meta">{question.featured && <FeaturedBadge kind="question" />}<span className="type-chip">{isSubjective ? uiText(locale, "主观题", "Short answer") : isMulti ? uiText(locale, "多选题", "Multiple choice") : uiText(locale, "单选题", "Single choice")}</span><span>{taxonomyText(question.taxonomy.learningLevel, locale)}</span><span>{textForLocale(question.taxonomy.modelFamily, locale)}</span></div>
      ...
      {revealed && <section className="answer-panel"><div className="answer-heading"><span>{uiText(locale, "参考答案", "Reference answer")}</span><strong>{question.correctAnswer.join("、") || uiText(locale, "见评分要点", "See guidance")}</strong></div>{!isSubjective && <p><RichText text={question.explanation} terms={terms} sourceNode={sourceNode} /></p>}{question.subjectiveAnswer && <div className="rubric"><p><RichText text={question.subjectiveAnswer.referenceAnswer} terms={terms} sourceNode={sourceNode} /></p><details className="scoring-details"><summary>{uiText(locale, "评分要点", "Scoring guidance")} · {subjectiveMaxScore(question)} {uiText(locale, "分", "points")}</summary><ol>{question.subjectiveAnswer.rubric.map((item, index) => <li key={`${index}-${textForLocale(item.criterion, locale)}`}><RichText text={item.criterion} terms={terms} sourceNode={sourceNode} />（{item.points} {uiText(locale, "分", "points")}）</li>)}</ol></details></div>}</section>}
      {question.ceptCheck && <section className="cept-check"><div className="cept-check-heading"><span className="cept-check-icon"><CeptCheckMark /></span><div><b>CeptCheck</b><span>{uiText(locale,"换个角度，再想一步","A new angle. A deeper understanding.")}</span></div>{question.ceptCheck.featured && <FeaturedBadge kind="ceptCheck" />}</div><h2><RichText text={question.ceptCheck.stem} terms={terms} sourceNode={sourceNode} /></h2></section>}
      {!preview && <footer className="question-footer" data-annotation-ignore>
        <button className="secondary-button" disabled={!previousId} onClick={() => previousId && navigate(`/learn/questions/${previousId}?mode=${mode}`)}><ChevronLeft size={17} /> {uiText(locale, "上一题", "Previous")}</button>
        ...
        <button className="primary-button" disabled={!nextId} onClick={() => nextId && navigate(`/learn/questions/${nextId}?mode=${mode}`)}>{uiText(locale, "下一题", "Next")} <ChevronRight size={17} /></button>
      </footer>}
    </article>
  );
}
