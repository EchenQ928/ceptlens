...
export function LearnCatalogPage() {
  ...
  }), [questions, family, level, engineering, status, query, progress, priority, extra, locale]);
  ...
  return (
    <div className="page catalog-page">
      ...
      <section className="filter-panel">
        <div className="search-row"><label className="search-box"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={uiText(locale, "搜索题目、概念或标签", "Search questions, concepts, or tags")} /></label>{first && <Link className="primary-button" to={`/learn/questions/${first.id}?mode=${mode}`}><BookOpenCheck size={17} /> {uiText(locale, "开始本题库", "Start library")}</Link>}</div>
        ...
        {advanced && <div className="advanced-filters"><label>{uiText(locale, "优先级", "Priority")}<select value={priority} onChange={event => setPriority(event.target.value)}><option value="all">{uiText(locale, "全部", "All")}</option>{["P0", "P1", "P2", "P3"].map(value => <option key={value}>{value}</option>)}</select></label>{extraFields.map(field => { const options = [...new Set(questions.flatMap(q => q.taxonomy[field.key] ?? []).map(value => textForLocale(value, "zh-CN")))]; return <label key={field.key}>{uiText(locale, field.label, ({ knowledgeTopics: "Knowledge topics", taskScenarios: "Task scenarios", optimizationObjectives: "Optimization objectives", runtimeEnvironments: "Runtime environments" }[field.key]))}<select value={extra[field.key] ?? ""} disabled={!options.length} onChange={event => setExtra(current => ({ ...current, [field.key]: event.target.value }))}><option value="">{options.length ? uiText(locale, "全部", "All") : uiText(locale, "当前内容未设置", "Not set")}</option>{options.map(value => <option key={value} value={value}>{taxonomyText(value, locale)}</option>)}</select></label>; })}</div>}
        <button className="advanced-trigger" onClick={() => { setQuery(""); setFamily("all"); setLevel("all"); setEngineering("all"); setStatus("all"); setPriority("all"); setExtra({}); }}><Filter size={15} /> {uiText(locale, "重置筛选", "Reset filters")}</button>
      </section>
      ...
      <section className="question-table" aria-label="题目列表">
        ...
        {filtered.map((question) => {
          ...
          return <Link className="question-list-row" to={`/learn/questions/${question.id}?mode=${mode}`} key={question.id}><span className="order-cell">{String(question.ordering.order).padStart(2, "0")}</span><span className="question-title-cell"><b>{richTextToPlainText(question.stem, locale)}</b><small>{textForLocale(question.taxonomy.primaryConcept, locale)}</small></span><span>{withoutCode(textForLocale(question.taxonomy.modelFamily, locale))}</span><span><i className="level-badge">{question.taxonomy.learningLevel.slice(0, 2)}</i></span><span>{withoutCode(question.taxonomy.engineeringStage)}</span><span><i className={`status-dot ${review ? "review" : complete ? "done" : ""}`} />{review ? uiText(locale, "待复习", "Review") : complete ? uiText(locale, "已完成", "Completed") : uiText(locale, "未学习", "Unseen")}</span><span><ArrowRight size={16} /></span></Link>;
        })}
...
function Facet({ label, value, onChange, options, count, labelFor = withoutCode }: { label: string; value: string; onChange: (value: string) => void; options: string[]; count?: (value: string) => number; labelFor?: (value: string) => string }) {
  ...
  return <div className="facet-row"><b>{label}</b><div className="facet-options"><button className={value === "all" ? "active" : ""} onClick={() => onChange("all")}>{uiText(locale, "全部", "All")}</button>{options.map((option) => <button key={option} className={value === option ? "active" : ""} onClick={() => onChange(option)}>{displayLabel(option)}{count && <small>{count(option)}</small>}</button>)}</div></div>;
}
