...
export function StudyPage() {
  ...
  return (
    <div className="study-layout">
      ...
      <aside className="study-context"><div className="context-section"><span className="eyebrow">POSITION</span><h3>{uiText(locale, "第", "Question")} {question.ordering.order}</h3><p>{textForLocale(question.taxonomy.primaryConcept, locale)}</p></div><div className="context-section"><span className="eyebrow">PREREQUISITES</span>{question.ordering.prerequisites.length ? question.ordering.prerequisites.map((id) => { const item = questions.find((candidate) => candidate.id === id); return item ? <Link key={id} to={`/learn/questions/${id}?mode=${mode}`}>{textForLocale(item.taxonomy.primaryConcept, locale)}</Link> : null; }) : <p>{uiText(locale, "本题无题目级前置。", "This question has no question-level prerequisite.")}</p>}</div><div className="context-section"><span className="eyebrow">TERM PACKAGES</span>{linkedTerms.length ? linkedTerms.map(({ dependency, term }) => term ? <Link key={term.id} to={`/terms/${term.id}`} state={{ termTrail: [questionNode, { kind: "term", id: term.id, label: textForLocale(term.title, locale), href: `/terms/${term.id}` }] }}>{textForLocale(term.title, locale)}<small><RichText text={term.summary} terms={terms} linkTerms={false} /></small></Link> : <div className="missing-term-card" key={dependency.id}><b>{termDisplayName(dependency.id, dependency.title, locale)}</b><span>{uiText(locale, "待导入教学包", "Package pending")}</span><small>{dependencyReason(dependency.reason, locale)}</small></div>) : <p>{uiText(locale, "本题没有显式词条链接。", "This question has no explicit term links.")}</p>}</div></aside>
    </div>
  );
}
