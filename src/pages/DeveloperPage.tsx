...
export function DeveloperPage() {
  ...
  async function importFiles(files: File[]) {
    ...
    await runMutation(() => kind === "question" ? contentHostClient.importQuestionFiles(files, token) : contentHostClient.importTermPackages(files, token));
  }
  ...
  return <div className="developer-layout">
    ...
    <section className="developer-main">
      ...
      {kind === "question" ? <>
        ...
        <div className="developer-toolbar"><button className="secondary-button" disabled={busy || !hostStatus || hostStatus.publishing} onClick={() => inputRef.current?.click()}><Upload size={16} /> {t("导入词条 ZIP（可多选）", "Import term ZIPs")}</button><input ref={inputRef} type="file" hidden accept="application/zip,.zip" multiple onChange={(event) => { const files = Array.from(event.target.files ?? []); event.target.value = ""; void importFiles(files); }} /><a className="secondary-button" href="api/content/templates/term"><Download size={16} /> {t("下载教学包模板", "Download package template")}</a><a className="secondary-button" href="api/content/terms/export"><Download size={16} /> {t("导出完整词条库", "Export term library")}</a><span className="toolbar-spacer" />{selectedTerm && <><a className="secondary-button" href={`api/content/terms/${selectedTerm.id}/export`}><Download size={16} /> {t("导出当前包", "Export package")}</a><button className="danger-button" disabled={busy || !hostStatus || hostStatus.publishing} onClick={remove}><Trash2 size={16} /> {t("删除", "Delete")}</button></>}</div>
      ...
      {result && <div role={result.ok ? "status" : "alert"} className={`validation-result ${result.ok ? "success" : "error"}`}><div>{result.ok ? <CheckCircle2 /> : <XCircle />}<strong>{result.ok ? t("发布完成", "Publish complete") : t("发布失败", "Publish failed")}</strong></div><ul>{result.messages.map((message) => <li key={message}>{message}</li>)}</ul></div>}
