...
import { Formula, termText, uiText, useLocale } from "@term-sdk";
...
function GraphNode({ node, frame, locale }: { node: NodeKey; frame: AnimationFrame; locale: "zh-CN" | "en-US" }) {
  ...
  return <g className={`${styles.graphNode} ${frame.activeNodes.includes(node) ? styles.graphNodeActive : ""}`} data-node={node}>
    ...
    <text className={styles.nodeLabel} x={layout.x + layout.width / 2} y={layout.y + 19} textAnchor="middle">{termText(layout.label, locale)}</text>
...
function NetworkGraph({ frame, frameIndex }: { frame: AnimationFrame; frameIndex: number }) {
  ...
  return <figure className={styles.graphFigure} aria-label={termText("反向传播计算图：前向值从输入流向损失，反向梯度从损失返回各个参数", locale)}>
    <div className={styles.graphLegend}><span><i className={styles.blueDot} />{uiText(locale, "前向值：从输入到 Loss", "Forward values: inputs to Loss")}</span><span><i className={styles.redDot} />{uiText(locale, "反向梯度：从 Loss 回到参数", "Backward gradients: Loss back to parameters")}</span></div>
    <svg className={styles.network} viewBox="0 0 1160 430" role="img" aria-label={termText("两个输入经过线性组合、激活和输出层得到损失，再沿同一计算图反向得到五个参数梯度", locale)}>
      <defs>
      ...
      {activeEdgeItems.map((edge) => <MovingBubble key={`${frameIndex}-${edge.key}`} edge={edge} direction={frame.activeDirection ?? "forward"} label={frame.activeDirection === "backward" ? "∂L" : forwardLabels[edge.key] ?? uiText(locale, "值", "Value")} id={`${frameIndex}-${edge.key}`} />)}
      {(Object.keys(nodeLayout) as NodeKey[]).map((node) => <GraphNode key={node} node={node} frame={frame} locale={locale} />)}
      {(Object.keys(parameterLayout) as Array<keyof Parameters>).map((key) => { const layout = parameterLayout[key]; const gradient = frame.gradients[key]; const active = frame.activeNodes.includes(key); return <g key={key} className={`${styles.parameterNode} ${active ? styles.parameterNodeActive : ""}`} data-parameter={key}><Bubble x={layout.x} y={layout.y} width={layout.width} height={layout.height} symbol={key} value={initial[key]} kind="parameter" gradient={gradient} active={active} /><text className={styles.parameterLabel} x={layout.x + layout.width / 2} y={layout.y - 9} textAnchor="middle">{termText(layout.label, locale)}</text></g>; })}
    </svg>
  </figure>;
}
...
export function BackpropagationAnimation() {
  ...
  const formula = locale === "en-US" ? frame.formula.replaceAll("后面传回的梯度", "upstream gradient").replaceAll("当前这一步的局部导数", "local derivative at this step") : frame.formula;
  ...
  return <div ref={stageRef} className={styles.animation} data-frame={index} data-phase={frame.phase}>
    <div className={styles.animationToolbar}><div className={styles.animationButtons}><button className={styles.primaryButton} onClick={play}>{playing ? uiText(locale, "暂停", "Pause") : last ? uiText(locale, "重新播放", "Replay") : uiText(locale, "播放演示", "Play demo")}</button><button onClick={() => seek(index - 1)} disabled={index === 0}>{uiText(locale, "上一步", "Previous")}</button><button onClick={() => seek(index + 1)} disabled={last}>{uiText(locale, "下一步", "Next")}</button><button className={styles.resetButton} onClick={() => seek(0)}>{uiText(locale, "回到完整图", "Back to full graph")}</button></div><span className={styles.progressLabel}>{index === 0 ? uiText(locale, "完整结构", "Full structure") : `${index} / ${frames.length - 1}`}</span></div>
    ...
    <div className={styles.animationExplanation} aria-live="polite"><span className={`${styles.phaseBadge} ${frame.phase === "backward" || frame.phase === "complete" ? styles.backwardBadge : ""}`}>{frame.phase === "overview" ? uiText(locale, "结构", "Structure") : frame.phase === "forward" ? uiText(locale, "前向传播", "Forward") : frame.phase === "backward" ? uiText(locale, "反向传播", "Backward") : uiText(locale, "完成", "Complete")}</span><h3>{termText(frame.title, locale)}</h3><p>{termText(frame.note, locale)}</p><div className={styles.formulaRow}><Formula expression={formula} symbols={[]} /><Formula expression={frame.substitution} symbols={[]} /></div></div>
    <p className={styles.animationHint}>{termText("同一张图贯穿全过程。蓝色值沿依赖关系向前，红色梯度沿相反方向返回；梯度到达参数后会停留在参数卡片上。", locale)}</p>
  </div>;
}
