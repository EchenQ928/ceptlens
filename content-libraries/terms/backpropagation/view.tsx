import { Callout, CodeBlock, EngineeringBoundaries, Formula, Paragraph, TermSection, defineTermView, uiText, useLocale } from "@term-sdk";
...
function TermBody() {
  ...
  return <div className={styles.root}>
    ...
    <TermSection id="forward-and-backward" title="先看一次前向，再看一次反向">
      ...
      <p className={styles.animationSummary}>{t("动画中的一次完整结果是：预测 ŷ=2，损失 L=1；反向传播得到 ∂L/∂w₁=−4、∂L/∂w₂=−2、∂L/∂b=−2、∂L/∂v=−6、∂L/∂c=−2。", "One complete run of the animation gives prediction ŷ=2 and loss L=1. Backpropagation produces ∂L/∂w₁=−4, ∂L/∂w₂=−2, ∂L/∂b=−2, ∂L/∂v=−6, and ∂L/∂c=−2.")}</p>
    </TermSection>
    ...
    <TermSection id="pytorch" title="在代码里，一次反向传播怎样发生">
      ...
      <details className={styles.details} open>
        <summary>{t("一个最小例子", "A minimal example")}</summary>
        <CodeBlock language="python" code={trainingCode} caption={t("loss.backward() 计算梯度；optimizer.step() 使用梯度更新参数。两步各自承担一段清晰的训练流程。", "loss.backward() computes gradients; optimizer.step() uses them to update parameters. Each step has a distinct role in the training loop.")} />
      </details>
    ...
    <TermSection id="autograd" title="框架为什么能自动完成反向传播">
      ...
      <div className={styles.autogradSteps}>
        <div><b>{t("前向时记录", "Record during the forward pass")}</b><span>{t("输入、运算和中间结果组成计算图。", "Inputs, operations, and intermediate results form the computation graph.")}</span></div>
        <div><b>{t("从 Loss 开始", "Start from Loss")}</b><span>{t("初始关系是 ∂L/∂L=1。", "The initial relationship is ∂L/∂L=1.")}</span></div>
        <div><b>{t("逐节点回传", "Propagate node by node")}</b><span>{t("上游梯度乘局部导数，并把结果交给更早的节点。", "Multiply the upstream gradient by the local derivative and pass the result to earlier nodes.")}</span></div>
        <div><b>{t("遇到分支就累加", "Add contributions at branches")}</b><span>{t("同一变量收到多条路径的梯度时，汇总所有贡献。", "When one variable receives gradients from multiple paths, sum all contributions.")}</span></div>
      </div>
