import { Callout, Formula, Paragraph, TermSection, defineTermView, uiText, useLocale } from "@term-sdk";
...
function ReluGraph() {
  ...
  return <figure className={styles.reluFigure} aria-label={t("ReLU 函数图像", "ReLU function plot")}>
    <figcaption><strong>{t("ReLU：负数归零，正数保留", "ReLU: negative values become zero; positive values stay")}</strong><span>{t("横轴是单元加权求和后的总和 z，纵轴是传出的结果 h。", "The x-axis is the unit's weighted sum z; the y-axis is the emitted result h.")}</span></figcaption>
    <svg className={styles.reluGraph} viewBox="0 0 640 310" role="img" aria-label={t("ReLU 在 z 小于等于 0 时输出 0，在 z 大于 0 时输出 z。示例 B 的总和 −1 对应输出 0，示例 A 的总和 2 对应输出 2。", "ReLU outputs 0 when z is at most 0 and z when z is greater than 0. Example B has sum −1 and output 0; example A has sum 2 and output 2.")}>
      ...
      <text className={styles.plotLabel} x="320" y="22" textAnchor="middle">{t("输出 h = ReLU(z)", "Output h = ReLU(z)")}</text><text className={styles.plotLabel} x="561" y="240">{t("总和 z", "Sum z")}</text>
      ...
      <text className={styles.plotAnnotation} x="185" y="185">{t("负数区间：h = 0", "Negative region: h = 0")}</text><text className={styles.plotAnnotation} x="462" y="162">{t("正数区间：h = z", "Positive region: h = z")}</text>
      <text className={styles.exampleLabel} x="265" y="288" textAnchor="middle">{t("B：−1 → 0", "B: −1 → 0")}</text><text className={styles.exampleLabel} x="455" y="114">{t("A：2 → 2", "A: 2 → 2")}</text>
    </svg>
    <p>{t("原点处输出也是 0。图中的折点把两种响应接在一起：左边是一条水平线，右边随输入等量上升。", "The output at the origin is also 0. The corner joins the two responses: a horizontal line on the left and a line rising with the input on the right.")}</p>
  </figure>;
}
...
function TermBody() {
  ...
  return <div className={styles.packageRoot}>
    <TermSection id="function-view" title="从外部看：一个函数">
      ...
      <div className={styles.functionBridge} aria-label={t("输入数字，经过神经网络内部的小计算单元，得到任务所需的结果", "Input numbers pass through the network's small computation units to produce the result needed by the task")}>
        <div><small>{t("输入", "Input")}</small><b>{t("表示数据的数字", "Numbers representing data")}</b></div><span aria-hidden="true">→</span><div className={styles.functionBox}><small>{t("神经网络", "Neural network")}</small><b>{t("许多相连的小计算单元", "Many connected computation units")}</b><em>{t("把计算结果继续传下去", "Pass computed results onward")}</em></div><span aria-hidden="true">→</span><div><small>{t("输出", "Output")}</small><b>{t("任务需要的结果", "The result required by the task")}</b></div>
      </div>
    ...
    <TermSection id="nonlinearity" title="为什么能表达复杂关系">
      <h3 className={styles.reasoningTitle}>{t("先把多个输入组合起来", "First combine multiple inputs")}</h3>
      ...
      <h3 className={styles.reasoningTitle}>{t("但固定比例的组合，也有表达边界", "Fixed-ratio combinations also have expressive limits")}</h3>
      ...
      <h3 className={styles.reasoningTitle}>{t("再用 ReLU 引入不同区间的响应", "Then use ReLU to introduce region-dependent responses")}</h3>
    ...
    <TermSection id="learning" title="参数是怎样学出来的">
      ...
      <div className={styles.trainingStory}><h3>{t("以“根据设备读数预测耗电量”为例", 'Example: predict power consumption from device readings')}</h3><ol><li><b>{t("先算一次预测。", "Make one prediction.")}</b>{t("把一组读数送入网络，用当前参数算出预测耗电量。", "Send a set of readings through the network and compute predicted power consumption with the current parameters.")}</li><li><b>{t("和已知结果比较。", "Compare with the known result.")}</b>{t("如果真实耗电量是 5 度，网络预测为 3 度，就还有差距。", "If actual consumption is 5 units and the network predicts 3, there is still a gap.")}</li><li><b>{t("调整参数，再尝试。", "Adjust parameters and try again.")}</b>{t("训练算法依据误差计算怎样小幅修改参数，再用许多这样的样本反复练习。", "The training algorithm uses the error to calculate small parameter changes, then repeats this process over many samples.")}</li></ol></div>
    ...
    <TermSection id="formula" title="把计算写成公式">
      ...
      <details className={styles.deeper}><summary>{t("进一步：同样的计算怎样写成通用形式？", "Further: how can the same computation be written in general form?")}</summary>
    ...
    <TermSection id="takeaway" title="总结">
      ...
      <p className={styles.source}>{t("进一步阅读：", "Further reading: ")}<a href="https://developers.google.com/machine-learning/crash-course/neural-networks/nodes-hidden-layers" target="_blank" rel="noreferrer">Google · {t("节点与隐藏层", "Nodes and hidden layers")}</a>{t("、", ", ")}<a href="https://developers.google.com/machine-learning/crash-course/neural-networks/activation-functions" target="_blank" rel="noreferrer">{t("激活函数", "Activation functions")}</a></p>
    </TermSection>
  </div>;
}
