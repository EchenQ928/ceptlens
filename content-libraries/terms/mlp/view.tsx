...
function TermBody() {
  ...
  return <div className={styles.root}>
    ...
    <TermSection id="layer-composition" title="层与层怎样配合">
      ...
      <div className={styles.resultChain}><div><small>{t("原始输入 x", "Raw input x")}</small><strong>[1, 2]</strong></div><span>→</span><div><small>{t("第一层结果 hA、hB", "Layer 1 results hA, hB")}</small><strong>[2, 0]</strong></div><span>→</span><div><small>{t("第二层结果 hC、hD", "Layer 2 results hC, hD")}</small><strong>[1, 4]</strong></div><span>→</span><div><small>{t("最终输出 y", "Final output y")}</small><strong>2.5</strong></div></div>
      <h3 className={styles.subheading}>{t("每层都在前一层的基础上提取特征", "Each layer extracts features from the previous layer")}</h3>
      ...
      <h3 className={styles.subheading}>{t("非线性让网络能够表达更复杂的关系", "Nonlinearity lets the network express more complex relationships")}</h3>
    ...
    <TermSection id="width-depth" title="宽度与深度">
      <div className={styles.dimensions}><div><h3>{t("宽度：一层有多少个单元", "Width: number of units in one layer")}</h3><p>{t("图中两个隐藏层的宽度都是 2。增宽某一层，就能在这一层同时产生更多个中间结果；不同层的宽度可以不同。", "Both hidden layers in the diagram have width 2. Widening a layer lets it produce more intermediate results at once; different layers can have different widths.")}</p></div><div><h3>{t("深度：经过多少层计算", "Depth: number of computation layers")}</h3><p>{t("图中有 2 个隐藏层，再加 1 个输出层。若按带参数的计算层计数，就是 3 层；输入层只接收数据，不计入这个数。", "The diagram has two hidden layers plus one output layer. Counting parameterized computation layers gives three; the input layer only receives data and is not counted.")}</p></div></div>
    ...
    <TermSection id="formula" title="把计算写成公式">
      ...
      <details className={styles.deeper}><summary>{t("进一步：任意宽度的一层怎样表示？", "Further: how do we represent a layer of any width?")}</summary>
    ...
    <TermSection id="pytorch" title="PyTorch 实现">
      ...
      <CodeBlock language="python" code={structureCode} caption={t("在已安装 PyTorch 的 Python 环境中运行。注释中的单元名称与上方网络图对应。", "Run this in a Python environment with PyTorch installed. The unit names in the comments correspond to the network diagram above.")} />
      <Paragraph>{'Linear 的两个数字分别是输入和输出特征数，前一层的输出数要与后一层的输入数一致。ReLU 保持形状不变。代码中的三个 Linear 才是带参数的计算层，合计 15 个参数。'}</Paragraph>
      <details className={styles.deeper}><summary>{t("复现图中的中间结果与输出 2.5", "Reproduce the intermediate results and output 2.5")}</summary>
        <Paragraph>{'上面的模型默认随机初始化参数，所以预测数值未必是 2.5。继续运行下面的代码，设置为图中的固定参数，就能逐层核对结果。'}</Paragraph>
        <CodeBlock language="python" code={reproduceCode} caption={t("切片 model[:2] 表示前两个模块；参数与原模型共用。这里只复现前向过程，不做训练。", "The slice model[:2] selects the first two modules and shares parameters with the original model. This only reproduces the forward pass; it does not train.")} />
      </details>
      <p className={styles.source}>{t("参考：", "References: ")}<a href="https://docs.pytorch.org/docs/stable/generated/torch.nn.Sequential.html" target="_blank" rel="noreferrer">PyTorch · Sequential</a></p>
    </TermSection>
    <TermSection id="summary" title="训练与总结">
      ...
      <p className={styles.source}>{t("进一步阅读：", "Further reading: ")}<a href="https://d2l.ai/chapter_multilayer-perceptrons/mlp.html" target="_blank" rel="noreferrer">{t("《动手学深度学习》· 多层感知机", "Dive into Deep Learning · Multilayer Perceptrons")}</a></p>
    </TermSection>
  </div>;
}
