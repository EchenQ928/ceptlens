...
const config = { enabled: true, baseUrl: 'https://example.invalid/v1', model: 'test-model', apiKeyEnv: 'CEPTLENS_TEST_KEY' };
...
beforeEach(() => vi.stubEnv('CEPTLENS_AI_DISABLED', ''));
afterEach(() => vi.unstubAllEnvs());
it('honors the explicit host disable flag even with provider configuration', async () => {
  vi.stubEnv('CEPTLENS_AI_DISABLED', '1'); vi.stubEnv('CEPTLENS_TEST_KEY', 'test');
...
it('uses the configured host only and excludes API credentials from frontend status', async () => {
  vi.stubEnv('CEPTLENS_TEST_KEY', 'secret-test-value');
...
it('validates every rubric point before accepting a score', async () => {
  vi.stubEnv('CEPTLENS_TEST_KEY', 'test');
...
it('redacts provider errors and refuses unregistered model tool calls', async () => {
  vi.stubEnv('CEPTLENS_TEST_KEY', 'secret');
