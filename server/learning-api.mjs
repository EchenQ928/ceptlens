  ...
  return {
    ...
    async handle(request, response, url, json) {
      ...
      try {
        ...
        if (url.pathname === '/api/auth/register' && request.method === 'POST') { const result = accounts.isAuthenticated(request.headers.cookie) ? accounts.register(data) : accounts.guestToAccount(user.id, data); response.setHeader('Set-Cookie', result.cookie); return send({ user: result.user }); }
        if (url.pathname === '/api/auth/login' && request.method === 'POST') { const result = accounts.login(data); response.setHeader('Set-Cookie', result.cookie); return send({ user: result.user }); }
        if (url.pathname === '/api/auth/logout' && request.method === 'POST') { const result = accounts.logout(request.headers.cookie); response.setHeader('Set-Cookie', result.cookie); return send({}); }
        if (url.pathname === '/api/auth/me' && request.method === 'GET') return send({ user, authenticated: !!accounts.authenticate(request.headers.cookie, () => null) });
        if (url.pathname === '/api/auth/providers' && request.method === 'GET') return send({ providers: accounts.providers() });
        if (url.pathname === '/api/progress' && request.method === 'GET') {
          if (!accounts.isAuthenticated(request.headers.cookie)) throw problem('请先登录后同步学习进度。', 401);
          return send({ progress: accounts.progress(user.id) });
        }
        if (url.pathname === '/api/progress' && request.method === 'POST') {
          if (!accounts.isAuthenticated(request.headers.cookie)) throw problem('请先登录后同步学习进度。', 401);
          if (!data || typeof data !== 'object' || Array.isArray(data)) throw problem('学习进度格式无效。');
