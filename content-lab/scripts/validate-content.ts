...
import { collectExplicitTermLinkDetails, formatZodErrors, questionPackageSchema, questionRichText, termPackageSchema, termRichText } from "../src/domain/schemas";
import { textForLocale } from "../src/domain/content";
...
for (const file of questionFiles) {
  ...
  if (!parsed.success) errors.push(...formatZodErrors(parsed.error).map((error) => `${file}: ${error}`));
    ...
    questionDependencies[parsed.data.id] = collectExplicitTermLinkDetails(questionRichText(parsed.data)).map((dependency) => ({
      ...dependency,
      reason: `题目“${textForLocale(parsed.data.taxonomy.primaryConcept, "zh-CN")}”中显式引用。`
    }));
  }
}
...
const termTitleById = new Map(terms.map((term) => [term.id, textForLocale(term.title, "zh-CN")]));
const termDependencies: Record<string, TermDependency[]> = {};
for (const term of terms) {
  const linked = collectExplicitTermLinkDetails(`${termRichText(term)}\n${termSourceById.get(term.id) ?? ""}`);
  const dependencies = new Map(linked.map((dependency) => [dependency.id, { ...dependency, reason: `词条“${textForLocale(term.title, "zh-CN")}”正文中显式引用。` }]));
  for (const prerequisiteId of term.prerequisites) if (!dependencies.has(prerequisiteId)) dependencies.set(prerequisiteId, {
    ...
    reason: `词条“${textForLocale(term.title, "zh-CN")}”将其声明为阅读前置。`
  });
...
for (const term of terms) for (const dependency of termDependencies[term.id] ?? []) recordMissing(dependency, `词条 ${textForLocale(term.title, "zh-CN")} · ${term.id}`);
