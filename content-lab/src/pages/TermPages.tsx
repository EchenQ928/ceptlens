...
export function TermLibraryPage() {
  ...
  return <div className="page">
    <div className="page-heading compact">
      <div><p className="eyebrow">SHARED KNOWLEDGE</p><h1>共享词条库</h1><p>已导入的本机教学包可进入预览；导入和技术校验不代表人工审查通过。</p></div>
      <div className="term-library-metrics"><span><strong>{terms.length + pendingTerms.length}</strong>全部词条</span><span><strong>{terms.length}</strong>已导入</span><span className="pending"><strong>{pendingTerms.length}</strong>待补</span></div>
    </div>
    <div className="term-library-toolbar">
      ...
      <div className="mode-segment" role="group" aria-label="词条状态">
        ...
        <button className={scope === "installed" ? "active" : ""} onClick={() => setScope("installed")}>已导入</button>
    ...
    <div className="term-grid">
      {installedResults.map((term) => <Link className="term-card" key={term.id} to={`/terms/${term.id}`}>
        <span className="content-status pending">本机草稿 · 待审查</span><h2>{term.title}</h2><p>{richTextToPlainText(term.summary)}</p><div><span>定制教学页面</span><ArrowRight size={16} /></div>
      </Link>)}
  ...
  return <div className="term-page-layout">
    <article className="term-article">
      ...
      <header className="term-hero">
        {!preview && <div className="term-hero-meta"><span className="content-status pending">本机草稿 · 待审查</span><span>{term.id}</span><span>SDK {term.sdkVersion}</span></div>}
        <h1>{term.title}</h1>
