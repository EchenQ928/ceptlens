...
export function LabPage() {
  ...
  return <div className="lab-workspace">
    <header className="lab-topbar"><Link className="lab-brand" to="/"><span className="brand-mark"><span /></span><strong>ModelPath <small>内容实验室</small></strong></Link><span className="lab-local">本机草稿 · 不连接正式站</span><a href="/docs/LAB_GUIDE.md" target="_blank" rel="noreferrer">开发指南</a></header>
...
export function LabActions({ kind, selectedId, existingTerms, existingQuestions, reload = (href?: string) => { if (href) window.location.hash = href; window.location.reload(); } }: { kind: Upload["kind"]; selectedId?: string; existingTerms: string[]; existingQuestions: string[]; reload?: (href?: string) => void }) {
  ...
  return <section className="lab-actions" aria-label="教学包操作">
    <div className="lab-action-row">
      ...
      <button className="secondary-button" disabled={!!busy || (kind === "terms" ? !selectedId : !existingQuestions.length)} onClick={() => void run("导出前校验、测试和构建…", async () => { await labClient.download(kind === "terms" ? `/api/content/terms/${selectedId}/export` : "/api/content/questions/export", kind === "terms" ? `${selectedId}.term.zip` : "modelpath-question-library.json"); setNotice("已导出。请到正式平台的内容管理输入口令后上传；实验室不会代为发布。"); })}><Download size={17} />{kind === "terms" ? "导出当前词条" : "导出题目包"}</button>
