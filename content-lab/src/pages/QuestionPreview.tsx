...
export function QuestionPreview({ question, terms }: { question: QuestionPackage; terms: TermPackage[] }) {
  const [selected, setSelected] = useState<string[]>([]); const [show, setShow] = useState(false); const [showCeptCheck, setShowCeptCheck] = useState(false);
  ...
  return <article className="question-panel"><div className="question-meta">{question.type === "subjective" ? "问答题" : question.type === "multiple_choice" ? "多选题" : "单选题"} · {question.taxonomy.learningLevel} · {question.taxonomy.modelFamily}</div><h1 className="question-stem">{rich(question.stem)}</h1>
    ...
    {question.ceptCheck && <section className="cept-check"><div className="question-meta">CeptCheck · 延伸理解</div><h2>{rich(question.ceptCheck.stem)}</h2><textarea className="subjective-answer" aria-label="试写 CeptCheck 答案" placeholder="用自己的话回答，不记录成绩" /><button className="secondary-button" onClick={() => setShowCeptCheck(value => !value)}>{showCeptCheck ? "隐藏参考" : "查看参考"}</button>{showCeptCheck && <div className="answer-panel"><p>{rich(question.ceptCheck.referenceAnswer)}</p><p>{rich(question.ceptCheck.explanation)}</p></div>}</section>}
  </article>;
}
