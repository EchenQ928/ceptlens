...
export function QuestionPreview({ question, terms }: { question: QuestionPackage; terms: TermPackage[] }) {
  ...
  return <article className="question-panel"><div className="question-meta">{question.type === "subjective" ? "问答题" : question.type === "multiple_choice" ? "多选题" : "单选题"} · {question.taxonomy.learningLevel} · {question.taxonomy.modelFamily}</div><h1 className="question-stem">{rich(question.stem)}</h1>
    ...
    {show && <section className="answer-panel"><div className="answer-heading"><span>参考解答</span>{question.type !== "subjective" && <strong>{question.correctAnswer.join("、")}</strong>}</div><p>{rich(question.type === "subjective" ? question.subjectiveAnswer!.referenceAnswer : question.explanation)}</p></section>}
    {question.ceptCheck && <section className="cept-check"><div className="question-meta">CeptCheck</div><h2>{rich(question.ceptCheck.stem)}</h2></section>}
  </article>;
}
